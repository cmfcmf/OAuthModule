...
namespace Cmfcmf\OAuthModule\Entity\Repository\Base;
 ...
 *
 * This is the base repository class for mapped id entities.
 */
class MappedId extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('index', 'view'))) {
                ...
                $listHelper = new ListEntriesUtil(ServiceUtil::getManager(), ModUtil::getModule('CmfcmfOAuthModule'));
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('mappedId', 'workflowState');
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['userId'] = (int) $this->request->query->get('userId', 0);
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Cmfcmf\OAuthModule\Entity\MappedIdEntity', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Cmfcmf\OAuthModule\Entity\MappedIdEntity', 'tbl')
    ...
    public function updateUserField($userFieldName, $userId, $newUserId)
    {
        ...
        if (empty($userFieldName) || !in_array($userFieldName, array('userId'))) {
     ...
     *
     * @return array|Cmfcmf\OAuthModule\Entity\MappedIdEntity retrieved data array or Cmfcmf\OAuthModule\Entity\MappedIdEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Cmfcmf\OAuthModule\Entity\MappedIdEntity instances
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('CmfcmfOAuthModule', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'mappedId', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'CmfcmfOAuthModule') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved mapped ids only
            $onlineStates = array('approved');
            $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('CmfcmfOAuthModule', 'showOnlyOwnEntries', 0), 'GETPOST');
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred mapped ids
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.claimedId LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.provider LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.workflowState = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.userId = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.claimedId LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.provider LIKE \'%' . $fragment . '%\'';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numMappedIds';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Cmfcmf\OAuthModule\Entity\MappedIdEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of mapped ids to exclude (optional).
     *
