...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Mapped id'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='mappedId' from=$items}
            <option value="{$mappedId.id}"{if $selectedId eq $mappedId.id} selected="selected"{/if}>{$mappedId->getTitleFromDisplayPattern()}</option>
        {foreachelse}
