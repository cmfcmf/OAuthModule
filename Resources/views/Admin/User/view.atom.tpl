{* purpose of this template: users atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='CmfcmfOAuthModule' type='admin' func='view' ot='user'}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='user' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='CmfcmfOAuthModule' type='admin' func='view' ot='user' fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$user.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='CmfcmfOAuthModule' type='admin' func='view' ot='user'}{/capture}
        <id>{$uniqueID}</id>
